...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.is_container == True and place.ant.can_contain(self):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not current_place.is_hive and self.upper_bound >= place_count:     
           if bee != None and place_count >= self.lower_bound:
               return bee
           else:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health - amount <= 0:
            do_damage += self.damage
            
        shallow_copy = list(self.place.bees)    
        for i in shallow_copy:
            i.reduce_health(do_damage)
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt eats random bee in its place and eats it whole, and chewing for 3 turns."""
    ...
    def action(self, gamestate):
        """If not chewing now, then can eat,or to chew."""
        if self.turns_to_chew == 0:
            eaten_bee = random_bee(self.place.bees)
            if eaten_bee != None:
                self.eat_bee(eaten_bee)
...
class TankAnt(ContainerAnt):
    """TankAnt not only provides protection but also offensive."""
    ...
    def action(self, gamestate):
        ...
        if bees != None:
            shallow_copy = list(bees)
            for i in shallow_copy:
                i.reduce_health(self.damage)   # tank's action
                
        super().action(gamestate)        # contained_ant's action
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_waterproof == False:          # remove not waterproof ants
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is waterproof thrower while will not lose health in water."""
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        curr_place = self.place.exit       # a queen inspires the ants behind her
        while curr_place:
            curr_ant = curr_place.ant
            
            if curr_ant:
                if curr_ant.is_container and curr_ant.ant_contained:
                    curr_ant.ant_contained.double()
                curr_ant.double()
