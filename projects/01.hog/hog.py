...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    if num_rolls == 0:
        return free_bacon(opponent_score) #function as returned value
    elif num_rolls > 0:
...
def swine_align(player_score, opponent_score):
    ...
    if a >= 10 and player_score > 0 and opponent_score > 0:
        return True
    else: 
        return False
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        ...
        s = [score0, score1]
        if extra_turn(s[who], s[other(who)]):
            who = who
        else:
            who = other(who)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    def comparison(score0, score1):
        if who == 0:
            new_score = score0
        else:
            new_score = score1
        gain = new_score - last_score
        if gain > running_high:
            print(gain,'point(s)! The most yet for Player', who)
            new_high = gain
        else:
            new_high = running_high
        return announce_highest(who, new_score, new_high) 
    return comparison
    # END PROBLEM 7
...
def make_averaged(original_function, trials_count=1000):
    ...
    def averaged_fun(*args):
        ...
        while i < trials_count:
            ...
            total_result += result
            i += 1
        averaged_result = total_result / trials_count
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    dice_num = 1
    while n <= 10:
        average_fun = make_averaged(roll_dice, trials_count)
        result = average_fun(n, dice)
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if swine_align(score + new_score, opponent_score) or pig_pass(score + new_score, opponent_score):
        return 0
    else:
