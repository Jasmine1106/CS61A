...
def balanced(m):
    ...
    if is_mobile(m):
        ...
        left_len, right_len = length(left_arm), length(right_arm)
        left_mass, right_mass = total_mass(end(left_arm)), total_mass(end(right_arm))
        
        if left_len * left_mass != right_len * right_mass:
            return False
        if is_mobile(end(left_arm)):
            if not balanced(end(left_arm)):
                return False
        if is_mobile(end(right_arm)):
            if not balanced(end(right_arm)):
                return False
        return True
...
def max_path_sum(t):
    ...
    if is_leaf(t):
        ...
        left_sum = max_path_sum(branches(t)[0])
        right_sum = max_path_sum(branches(t)[1])
    return label(t) + max(left_sum, right_sum)
